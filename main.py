...
def coordin(ques_coords):
    ...
    yu, yd = lowest_y_center[1] - 2, highest_y_center[1] + 13
...
def ans_box(image, yu, yd, xl, xr, len_ques, start_index, output_dir):

    height = math.floor( (yd - yu) / len_ques)
    ...
    for i in range(len_ques):
        ...
        y1 = yu + i * height
        y2 = yu + (i + 1) * height
        ques_box = image[y1:y2, xl:xr]

        filename = f"{output_dir}/question_{idx}.png"
        cv2.imwrite(filename, ques_box)
...
def one_main(image_path, rec_mark_path, ques_mark_path):
    ...
    yu1, yd1, xl1, xr1, len_ques_p1 = coordin(ques_coords_part1)
    yu2, yd2, xl2, xr2, len_ques_p2 = coordin(ques_coords_part2)
    ...
    output_dir = "/home/matin/Desktop/answers_box"
    ans_box(image, yu1, yd1, xl1, xr1, len_ques_p1,  start_index=0, output_dir=output_dir)
    ans_box(image, yu2, yd2, xl2, xr2, len_ques_p1, start_index=25, output_dir=output_dir)


    full_score = score_p1 + score_p2
    ans_dict = answer_dict(correct_p1, correct_p2)
    len_ques = len_ques_p1 + len_ques_p2

    print("User Answers:")
    print(ans_dict)
    print(f"Score Part 1: {score_p1}")
    print(f"Score Part 2: {score_p2}")
    print(f"Full Score: {full_score} / {len_ques}")
